...
MainGame::~MainGame(){
	for (int i = 0; i < _ballRenderers.size(); i++){
		delete _ballRenderers[i];
	}
}
...
void MainGame::initRenderers(){
	...
	_ballRenderers.push_back(new MomentumBallRenderer);
	_ballRenderers.push_back(new VelocityBallRenderer(m_screenWidth, m_screenHeight));
}

struct BallSpawn {
    BallSpawn(const Engine::Color& colr,
...
void MainGame::update(float deltaTime) {
    m_ballController.updateBalls(m_balls, _grid.get(), deltaTime, m_screenWidth, m_screenHeight);
}
...
void MainGame::drawHud() {
    const Engine::Color fontColor(255, 0, 0, 255);
    // Convert float to char *
    char buffer[64];
    sprintf(buffer, "%.1f", m_fps);

    m_spriteBatch.begin();
    m_spriteFont->draw(m_spriteBatch, buffer, glm::vec2(0.0f, m_screenHeight - 40.0f),
...
void MainGame::processInput() {
	...
	if (m_inputManager.isKeyPressed(SDLK_1)){
		_currentRenderer++;
		if (_currentRenderer >= _ballRenderers.size()){
			_currentRenderer = 0;
		}
	...
	if (m_inputManager.isKeyPressed(SDLK_s)){
		for (int i = 0; i < m_balls.size(); i++){
			m_balls[i].velocity = glm::vec2(0.0f, 0.0f);
		}
	}
}